# Labonair Development MCP Server

This directory contains a Model Context Protocol (MCP) server that provides Playwright browser automation capabilities for Labonair development and testing. The MCP server exposes Labonair's Playwright testing infrastructure through a standardized interface, allowing AI assistants and other tools to interact with browsers programmatically.
...
│   ├── playwright.ts        # Labonair Playwright integration
...
- **Labonair Integration**: Uses Labonair's existing Playwright test infrastructure
...
### Server Won't Start
- Ensure Labonair has been built and run at least once (via F5 or `code.sh`)
...
This MCP server is part of the Labonair development infrastructure. When making changes:
