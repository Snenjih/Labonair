...
pub async fn font_import(source_path: String, label: String) -> Result<CustomFontInfo, String> {
    ...
    if label.chars().count() > MAX_LABEL_LEN {
        return Err(format!("Font name must be {} characters or fewer", MAX_LABEL_LEN));
    }
    ...
    let dest = if dest.exists() {
        let stem = source.file_stem().and_then(|s| s.to_str()).unwrap_or("font");
...
pub async fn fonts_list_system() -> Result<Vec<String>, String> {
    tokio::task::spawn_blocking(|| SYSTEM_FONTS_CACHE.get_or_init(scan_system_fonts_sync).clone())
        .await
